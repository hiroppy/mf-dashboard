...
- **静的再生成**: crawler 完了後、web コンテナの `/api/refresh/` を Docker bridge 経由で Bearer 認証付き POST → `revalidatePath('/', 'layout')` で全ルートを invalidate。次のリクエストで新しい DB の内容を反映 (`expose:` のみで host には公開しないので外部到達不可)
...
通知を skip する。Docker Compose 用の `WEB_URL=http://web:8765` は `pnpm setup:cloudflare`
または `compose.yml` が設定する。
