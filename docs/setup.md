...
- MoneyForward でワンタイムパスワードの設定を行う ([参考](https://support.me.moneyforward.com/hc/ja/articles/7359917171481-%E4%BA%8C%E6%AE%B5%E9%9A%8E%E8%AA%8D%E8%A8%BC%E3%81%AE%E8%A8%AD%E5%AE%9A%E6%96%B9%E6%B3%95))
  - 後続の環境変数設定で使うため、設定時に表示される TOTP secret を控える
...
発行した token はパスワードマネージャーなどへ保管したうえで、実値を gitignore 済みの `terraform/terraform.tfvars` の `cloudflare_api_token` に設定する。Terraform は `.env` からインフラ設定を読み取らない。
...
`.env` を作成し、MoneyForward の必須値を設定する:
...
| `MONEY_FORWARD_EMAIL`                        | ✅       | MoneyForward のログインメールアドレス                                                                 |
| `MONEY_FORWARD_PASSWORD`                     | ✅       | MoneyForward のログインパスワード                                                                     |
| `MONEY_FORWARD_TOTP_SECRET`                  | ✅       | TOTP secret (base32 または `otpauth://` URI)                                                          |
...
`MONEY_FORWARD_TOTP_SECRET` には、ワンタイムパスワード設定時に表示されるセットアップキーを指定する。すでに設定済みで secret が分からない場合は、MoneyForward 側でワンタイムパスワードを再設定して新しい secret を保存する。
