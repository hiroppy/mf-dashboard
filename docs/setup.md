...
| AI_PROVIDER      |          | openai / anthropic / google | LLM fallbackをActionsで使う場合に必要                             |
| AI_MODEL         |          | 利用するモデル名            | LLM fallbackをActionsで使う場合に必要                             |
...
| CATEGORY_RULES_JSON      |          | `data/category-rules.json` のJSON本文            | Actionsで未分類取引のカテゴリ決定を有効化する場合に必要 |
| AI_API_KEY               |          | 利用するLLM providerのAPI key                    | LLM fallbackをActionsで使う場合に必要                   |
...
`data/category-rules.json` を作成すると、crawlerはDB保存前に新規の未分類取引のカテゴリ決定を試みる。ファイルが存在しない場合、この機能は無効になり、未分類のまま従来通り保存する。

```sh
$ cp data/category-rules.example.json data/category-rules.json
```

`data/category-rules.json` は `.gitignore` 対象なのでコミットしない。GitHub Actionsの定期実行で使う場合は、Secret `CATEGORY_RULES_JSON` に `data/category-rules.json` のJSON本文を設定し、crawler実行前にファイルを復元するstepをworkflowへ追加する。

```yaml
- name: Restore category rules config
  env:
    CATEGORY_RULES_JSON: ${{ secrets.CATEGORY_RULES_JSON }}
  run: |
    if [ -n "$CATEGORY_RULES_JSON" ]; then
      mkdir -p data
      printf '%s\n' "$CATEGORY_RULES_JSON" > data/category-rules.json
    fi
```

Secretが未設定の場合はファイルが作られず、カテゴリ決定機能は無効のまま動作する。
...
- confidenceが `minConfidence` 未満のLLM結果はMoneyForwardへ反映しない。
- 採用したカテゴリはMoneyForwardの `/cf/update` へ反映し、対象月を再スクレイプしてDBへ保存する。
- 更新に失敗してもcrawlerは停止せず、対象取引は未分類のまま保存する。

LLM fallbackを使う場合は、`llm.enabled` を `true` に変更し、既存のAI設定として `AI_PROVIDER` / `AI_MODEL` / `AI_API_KEY` も設定する。GitHub Actionsの定期実行で使う場合は、`AI_PROVIDER` と `AI_MODEL` をVariables、`AI_API_KEY` をSecretに設定し、crawler stepの `env` へ渡す。
...
LLMには取引の日付、種別、金額、内容、候補カテゴリID/名称を送るため、外部providerへ送信してよい場合だけ有効にする。専用のカテゴリ決定envはなく、機能ON/OFFは `data/category-rules.json` の有無で決まる。
