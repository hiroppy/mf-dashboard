...
`.env`では単一引用符で囲んだ値はそのまま扱われる。パスワードに`$`が含まれる場合の意図しない環境変数展開を防ぐため、`MF_PASSWORD`は単一引用符で囲む。Node.jsとDocker Composeでは単一引用符のエスケープ方法が異なるため、パスワード自体に単一引用符を含む場合はこの記法を使用できない。
