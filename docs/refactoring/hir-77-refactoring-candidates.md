...
- monthly / yearly / category 集計が同じ classified transfer result を使うようにする
...
また `pnpm knip` は root `package.json` の `lefthook` を unused devDependency として検出する。`pnpm lint` は exit 0 だが、a11y warning、typed mock warning、conditional expect warning、disabled e2e warning が残っている。
...
- `lefthook` を利用するなら config を追加し、利用しないなら依存から削除する
...
- `mf-dashboard: 資産履歴カテゴリ取得を一括クエリ化する`
